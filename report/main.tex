...
for distributional fidelity also improves privacy when the generator
memorizes: on the tabular VAE it increased the distance between synthetic and
real records by 17 to 26\% on all three datasets while also improving utility,
and on a CTGAN that does not memorize the same terms gave no privacy gain.
Second, whether an explicit privacy penalty costs
...
\citep{zhao2021ctabgan}, which detects memorization by comparing the first
...
clean \texttt{DataFrame} without missing values and with a binary target,
behind a single \texttt{DatasetBundle} interface, so that no downstream code
depends on which dataset it is handling. The loaders preserve semantic types:
continuous measurements remain numerical, categorical attributes are kept as
strings, and binary attributes and targets are normalized to boolean values.
The categorical values are therefore not label-encoded into arbitrary integer
codes at the loader stage; categorical handling is delegated to the generator
transformers.

The baseline and loss-aware experiments share this dataset-loading stage and
the same stratified train/test split. They differ only at the generator layer.
The SDV baselines construct an SDV metadata object and let the SDV data
processor select RDT transformers. The loss-aware CTGAN bypasses the SDV
synthesizer because its training loop must be modified directly; it instead
uses CTGAN's native \texttt{DataTransformer}, with discrete columns supplied
by the project's semantic-type and low-cardinality inference rule. Thus, the
two pipelines preserve the same distinction between discrete and continuous
features, but implement it through different library interfaces.
...
For Adult, the income target is normalized to a boolean and the sex column is
normalized from its textual values to a boolean. For Diabetes, ``?'' entries
are treated as missing, columns with more than 40\% missing values are dropped,
remaining incomplete rows are removed, the readmission target is binarized to
``readmitted within 30 days'', and the admission, discharge and source ID
codes are stored as strings because they represent categories rather than
continuous measurements. For Heart Disease, the target is binarized as
``disease present'' (the original severity score is positive), the binary
columns \texttt{sex}, \texttt{fbs} and \texttt{exang} are stored as booleans,
and the encoded categorical columns \texttt{cp}, \texttt{restecg},
\texttt{slope}, \texttt{ca} and \texttt{thal} are stored as strings. No
one-hot encoding is performed by the loaders. During CTGAN and TVAE fitting,
the generator-side transformer one-hot encodes the columns inferred as
discrete and applies mode-specific normalization to continuous columns.
...
\subsection{The same objective on CTGAN}
\label{sec:lossaware:ctgan}

Section~\ref{sec:results:ctgan} also tests the three penalties on CTGAN, to
check whether the effects found on TVAE depend on the base architecture.
CTGAN \citep{xu2019ctgan} trains a generator $G_\theta$ and a discriminator
$D_\psi$ with a conditional Wasserstein objective; only the generator side is
modified. With $\tilde x = \sigma(G_\theta(z, c))$ for a sampled condition
vector $c$, the stock generator loss is
\begin{equation}
\mathcal{L}_G^{\text{stock}} = -\mathbb{E}\big[D_\psi(\tilde x, c)\big] + \text{CE}(\hat c, c),
\end{equation}
the critic score plus a cross-entropy term that keeps the generated row
consistent with the sampled condition. The loss-aware generator objective
adds the same three terms as Equation~\ref{eq:objective}, applied to the same
fake batch $\tilde x$ and a real batch drawn under the matching condition:
\begin{equation}
\mathcal{L}_G = \mathcal{L}_G^{\text{stock}} + \lmmd\,\Lmmd + \lcorr\,\Lcorr + \lpriv\,\Lpriv .
\label{eq:objective-ctgan}
\end{equation}
$\Lmmd$, $\Lcorr$ and $\Lpriv$ are defined exactly as before; only the base
term they are added to differs (a WGAN critic score instead of a VAE
reconstruction and KL term), and the discriminator's own loss and gradient
penalty are untouched. Because the critic score has an arbitrary scale, the
same numeric $\lpriv$ does not carry the same relative weight it did on TVAE;
we come back to this in \S\ref{sec:limitations}. Algorithm~\ref{alg:lossaware-ctgan}
in Appendix~\ref{app:algorithm} gives the full training loop.
...
in this report can be reproduced from disk. The orchestration layer is kept
separate from both the generators and the metrics: \texttt{run\_experiment()}
loads and splits a dataset, obtains the requested model from the central
generator registry, trains or reloads it, samples synthetic rows, and passes
the result to the evaluation and reporting functions. The registry maps the
same names used in the configuration and in the command line to objects that
share a common \texttt{fit}/\texttt{sample} interface, so adding a generator
does not require changes to the experiment pipeline. Persistence stores the
trained model, the detected SDV metadata, the complete JSON report and an
append-only \texttt{index.jsonl}; this supports both exact re-evaluation of a
saved model and later querying of runs without reopening every report. Finally,
\texttt{src/main.py} exposes the workflow through four CLI commands: \texttt{run}
for training and evaluation, \texttt{evaluate} for a persisted model,
\texttt{show} for inspecting a report, and \texttt{find} for filtering the run
index. Generator parameters are passed through repeatable \texttt{--kwarg
KEY=VALUE} options, keeping the experimental protocol explicit and avoiding
dataset- or architecture-specific logic in the command interface.

The TVAE loss-aware runs use \texttt{batch\_size=32} and 300 epochs for Heart,
and \texttt{batch\_size=500} and 100 epochs for Adult and Diabetes. The
loss-aware CTGAN Heart comparison uses \texttt{batch\_size=50}, 2,000 epochs,
and three seeds ($1,2,3$); each seed is run under the baseline, utility-only,
and both-penalties configurations. Unless overridden on the command line,
each run samples 1,000 synthetic rows. The utility weights are fixed at
$\lmmd = 1$, $\lcorr = 0.5$ whenever they are enabled. We report means across
seeds. Wherever a $\pm$ appears it is the half-width of the 95\%
...
evaluation pipeline without modifications: CTGAN and TVAE via the
\texttt{sdv} library's \texttt{CTGANSynthesizer} and \texttt{TVAESynthesizer}
(which wrap the same \texttt{ctgan} package models used directly in
\S\ref{sec:lossaware}, with \texttt{sdv}'s metadata-driven preprocessing on
top), and a Gaussian copula, which is a purely statistical model without a
neural network that fits marginals and a correlation matrix.
...
\S\ref{sec:results:collapse}. Its Heart row is not usable as reported; with a
tuned training budget (\S\ref{sec:results:ctgan}) CTGAN reaches F1
discrepancy $0.01$ on Heart while keeping DCR $2.6$.
...
\subsection{Does the effect depend on the generator? Loss-aware CTGAN on Heart}
\label{sec:results:ctgan}

The baseline comparison (\S\ref{sec:results:baselines}) showed TVAE to be the
least private of the three architectures, which raised the possibility that
the privacy gain from the utility terms was a TVAE property rather than a
general one. To test this we added the same three penalty terms to the
generator objective of CTGAN (the discriminator is unchanged) and ran the
three main configurations on Heart Disease with \texttt{batch\_size=50} and
2{,}000 epochs, about 8{,}000 generator steps, which is where the stock model
converges on this dataset. The implementation is in
\texttt{src/generators/loss\_aware\_ctgan.py}.

\begin{table}[htbp]
\caption{Loss-aware CTGAN on Heart Disease, 3 seeds, $\mu = 1.5$. $\pm$ is
the 95\% CI half-width. TVAE rows from Tables~\ref{tab:heart-ablation} and
\ref{tab:heart-sweep} for comparison.}
\label{tab:ctgan-lossaware}
\centering\small
\begin{adjustbox}{max width=\textwidth}
...
\midrule
CTGAN & baseline      & $0.012 \pm 0.082$ & $2.57 \pm 0.46$ & $2.14 \pm 0.78$ & $2.59 \pm 0.25$ & 1.37 & 0.89 \\
CTGAN & utility only  & $0.005 \pm 0.048$ & $3.14 \pm 0.75$ & $1.88 \pm 0.45$ & $2.54 \pm 0.08$ & 1.34 & 0.88 \\
CTGAN & both ($\lpriv = 10$) & $0.044 \pm 0.068$ & $17.3 \pm 4.1$ & $2.38 \pm 0.97$ & $5.03 \pm 0.42$ & 3.61 & 0.94 \\
\midrule
TVAE  & baseline      & $-0.03 \pm 0.09$ & $1.36 \pm 0.44$ & $2.66 \pm 0.55$ & $1.72 \pm 0.13$ & 0.74 & 0.83 \\
TVAE  & utility only  & $-0.01 \pm 0.06$ & $1.25 \pm 0.54$ & $1.91 \pm 0.74$ & $2.02 \pm 0.10$ & 1.01 & 0.85 \\
TVAE  & both ($\lpriv = 10$) & $0.07 \pm 0.08$ & $2.46 \pm 0.62$ & $2.28 \pm 0.32$ & $2.73 \pm 0.23$ & 1.45 & 0.88 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

\paragraph{The utility terms do not improve privacy on CTGAN.} Mean DCR goes
from $2.59$ to $2.54$, with overlapping intervals. Correlation distance
improves, as it should since the term is in the loss, and MMD moves slightly,
but there is no privacy gain. This settles the question raised in
\S\ref{sec:results:baselines}: the effect is specific to TVAE, and the reason
is visible in the table. A properly trained CTGAN starts at DCR $2.59$, which
is already above the level the utility terms lifted TVAE to ($2.02$). TVAE at
$1.72$ was partially memorizing its training rows; the utility terms corrected
that; CTGAN did not have the problem in the first place. The memorization
hypothesis we set aside in \S\ref{sec:results:adult} therefore returns in a
more precise form: fidelity regularization acts as a privacy mechanism for
generators that memorize, and its effect is bounded by how far below the
non-memorizing level the baseline sits. On TVAE it was 17--26\% on all three
datasets; on CTGAN it is zero.

\paragraph{The hinge works on CTGAN, and lands harder.} At $\lpriv = 10$ mean
DCR nearly doubles ($2.59 \to 5.03$), the 5th percentile goes from $1.37$ to
$3.61$, and NNDR reaches $0.94$, the off-manifold signature of
\S\ref{sec:results:collapse}. Mean EMD rises seven-fold while F1 discrepancy
barely moves ($0.012 \to 0.044$, interval including zero). This is the
collapse pattern that TVAE showed at $\lpriv = 10$, $\mu = 2$, arriving one
step earlier, and it is consistent with the hinge-scale confound in
\S\ref{sec:limitations}: the WGAN generator loss has a different scale from a
VAE reconstruction loss, so the same numeric $\lpriv$ is a stronger penalty
here. A sweep at $\lpriv \in \{2, 5\}$ would locate CTGAN's knee; we did not
have time to run it.

\paragraph{A properly trained CTGAN is a good Heart baseline.} With a batch
size that gives several steps per epoch and a budget of 8{,}000 steps, CTGAN
reaches F1 discrepancy $0.01$ and correlation distance $2.14$ (better than
TVAE's $2.66$) at DCR $2.59$ (much more private than TVAE's $1.72$), at the
cost of a worse EMD ($2.57$ against $1.36$). The $\dagger$ row of
Table~\ref{tab:baselines} was an artefact of the library default batch size
and should be read against this one.
...
distributional fidelity also improved every privacy indicator on TVAE, by 17
to 26\% in mean DCR. Regularizing towards population-level statistics spreads
...
The loss-aware CTGAN experiment (\S\ref{sec:results:ctgan}) shows the limit of
this: on a generator that does not memorize to begin with, the same terms give
no privacy gain. The mechanism is a correction of memorization, and it does
as much good as there is memorization to correct. The common picture of two
quantities on opposite ends of a dial still misses this region, which exists
for the most widely used tabular VAE on all three datasets we tried.
The per-feature analysis makes the region narrower (on Adult it does not
reach $\lpriv = 10$) but does not remove it: at $\lpriv = 2$ on Diabetes the
diagnosis codes, the gender marginal and the privacy indicators all improved
together.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Two float-typed categoricals were modelled as continuous.}
        On Heart Disease, \texttt{ca} (4 values) and \texttt{thal} (3 values)
        are stored as floats, and the discrete-column inference used in the
        TVAE experiments only declared integer columns discrete. Both were
        therefore passed through mode-specific normalization rather than
        one-hot encoding, and the generator could emit non-integer values for
        them. The inference rule has since been corrected (bool, object and
        low-cardinality whole-number columns are all treated as discrete); the
        TVAE results in this report predate the fix. The effect is limited to
        two of Heart's thirteen features and applies equally to every
        configuration, so it does not change the comparisons, but it is a
        real fidelity loss on those columns.
  ...
  \item \textbf{Three datasets; loss-aware penalties on two generators, one
        of them on one dataset only.} The density claim rests on three points
        on one axis, all from TVAE. The loss-aware CTGAN experiment
        (\S\ref{sec:results:ctgan}) covers Heart only, at a single
        $\lpriv$ that turned out to be past CTGAN's knee, so it establishes
        that the utility-term privacy gain is TVAE-specific but does not
        locate CTGAN's own trade-off curve. The Gaussian copula has no
        gradient-based training and cannot be made loss-aware in this sense.
...
\bigskip
Algorithm~\ref{alg:lossaware-ctgan} is the corresponding loop for
\texttt{LossAwareCTGAN.fit()} (\S\ref{sec:lossaware:ctgan}). The discriminator
steps are unchanged from \texttt{ctgan.CTGAN}; the inserted block (lines
11--16) mirrors Algorithm~\ref{alg:lossaware}'s penalty block applied to the
generator's own fake batch rather than a fresh prior sample, since CTGAN
already produces one every step.

\begin{algorithm}[htbp]
\caption{Loss-aware CTGAN training (generator step only; discriminator steps unchanged from stock CTGAN)}
\label{alg:lossaware-ctgan}
\begin{algorithmic}[1]
\Require training table $D$; discrete columns; weights $\lmmd, \lcorr, \lpriv$; relative margin $\mu$; bandwidth $\gamma$
\State $T \gets \textsc{DataTransformer}(D)$; \; $X \gets T(D)$; initialise $G_\theta$, $D_\psi$, optimisers
\State $\tilde d_{\text{real}} \gets \operatorname{median}_i \min_{j \ne i} \|X_i - X_j\|_2$; \; $m \gets \mu \cdot \tilde d_{\text{real}}$
\For{epoch $= 1 \dots E$}
  \For{each step}
    \State \Comment{discriminator steps: unchanged from stock CTGAN, omitted here}
    \State $c \gets$ sampled condition vector; $z \sim \mathcal N(0,I)$
    \State $\tilde x_{\text{raw}} \gets G_\theta(z, c)$; \; $\tilde x \gets \sigma(\tilde x_{\text{raw}})$ \Comment{$\tanh$ / softmax, as in Alg.~\ref{alg:lossaware}}
    \State $\mathcal L_G \gets -D_\psi(\tilde x, c) + \text{CE}(\hat c, c)$ \Comment{stock CTGAN generator loss}
    \If{$\lmmd > 0$ or $\lcorr > 0$ or $\lpriv > 0$}
      \State $x \gets$ real batch sampled under condition $c$
      \If{$\lmmd > 0$ or $\lcorr > 0$}
        \State $\mathcal L_G \gets \mathcal L_G + \lmmd \cdot \textsc{MMD}^2_\gamma(\tilde x, x) + \lcorr \cdot \|\mathrm{Corr}(\tilde x) - \mathrm{Corr}(x)\|_F$
      \EndIf
      \If{$\lpriv > 0$}
        \State $\tilde x^{\text{hard}} \gets \sigma_{\text{hard}}(\tilde x_{\text{raw}})$; \; $d_i \gets \min_j \|\tilde x^{\text{hard}}_i - x_j\|_2$
        \State $\mathcal L_G \gets \mathcal L_G + \lpriv \cdot \frac1B \sum_i \max(0, m - d_i)$
      \EndIf
    \EndIf
    \If{$\mathcal L_G$ is not finite} \textbf{skip step} \EndIf
    \State backpropagate $\mathcal L_G$; generator optimiser step
    \State log $(\mathcal L_G, \mathcal L_D, \Lmmd, \Lcorr, \Lpriv, \bar d)$
  \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
...
\texttt{sdv} 1.32.1, \texttt{scikit-learn} 1.9, on Apple silicon (MPS).
